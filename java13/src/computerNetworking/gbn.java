package computerNetworking;// Java Applet Demonstration of Go-Back-N Protocol.
...
public class gbn extends Applet implements ActionListener, Runnable    
  ...
  while (currenthread==gbnThread)   // While the animation is running    
   if (onTheWay(sender))    // Checks if any of the packets are travelling    
    ...
    for (int i=0; i<total_packet; i++)    
     if (sender[i]!= null)    
      if (sender[i].on_way)     // If computerNetworking.packet is roaming
       if (sender[i].packet_pos < (v_clearance-pack_height))    
        sender[i].packet_pos+=5;  // Move computerNetworking.packet
 ...
 public void actionPerformed(ActionEvent e)    
  ...
  if (cmd == "rdt" && nextseq < base+window_len) // Send button is pressed    
   ...
   if (base == nextseq)    
    ...
    statusMsg += " Timer set for computerNetworking.packet "+ base +".";
    if (timerThread == null)    
     timerThread = new Thread(this);    
    timerSleep = true;    
    timerThread.start();    
   ...
   if (sender[selected].packet_ack)    
    statusMsg = "Packet "+ selected +" destroyed. Timer running for computerNetworking.packet "+base+".";
   else    
    statusMsg = "Acknowledgement of computerNetworking.packet "+ selected +" destroyed. Timer running for computerNetworking.packet "+base+".";
  ...
  if (sender[i]!= null)    
   ...
   if (x>=xpos && x<= xpos+pack_width && sender[i].on_way)    
    ...
    if ((sender[i].packet_ack && y>=v_offset+ypos && y<=v_offset+ypos+pack_height) || ((!sender[i].packet_ack) && y>=v_offset+v_clearance-ypos && y<=v_offset+v_clearance-ypos+pack_height))    
     ...
     statusMsg = "Click on a moving computerNetworking.packet to select.";
    }    
   else    
    statusMsg = "Click on a moving computerNetworking.packet to select.";
   }    
 ...
 // checkes all the packets before packno. Returns false if any computerNetworking.packet has
